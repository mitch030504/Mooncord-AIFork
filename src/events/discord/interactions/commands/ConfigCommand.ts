...
export default class ConfigCommand extends BaseCommand {
    ...
    private async uploadConfiguration(interaction: ChatInputCommandInteraction) {
        ...
        const uploadRequest = await uploadAttachment(attachment as any, 'config', directory)

        if (uploadRequest) {
            await interaction.editReply(this.locale.messages.answers.upload_successful
                .replace(/(\${filename})/g, attachment?.name || '')
                .replace(/(\${username})/g, interaction.user.tag))
            const serviceRestart = await this.serviceHelper.restartServiceByFile(attachment?.name as string)
